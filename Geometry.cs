...
{
	/// <summary>
	/// Static class to provide geometric utility functions on points
	/// </summary>
	public static class Geometry
	{
		...
		/// <summary>
		/// True if two values are "essentially" equal (i.e., equal within tolerance).
		/// </summary>
		...
		/// <summary>
		/// True if two points are "essentially" equal
		/// </summary>
		...
		/// <summary>
		/// Determines if a number is equal to zero within tolerance.
		/// </summary>
		...
		/// <summary>
		/// Signed area of the triangle between three points.  This routine is fundamental
		/// to a number of other geometry routines.  It's positive if the three points are
		/// in counterclockwise order, negative otherwise and it's absolute value is the
		...
		/// <summary>
		/// Determine if pt1, pt2, pt3 occur in Counter Clockwise order
		/// </summary>
		/// <param name="pt1">First point</param>
		/// <param name="pt2">Second point</param>
		/// <param name="pt3">Third point</param>
		...
		/// <summary>
		/// Determine if the test point is to the left of the line looking
		/// from ptSegmentStart to ptSegmentEnd.
		/// </summary>
		...
		/// <summary>
		/// Determine if three points are essentially collinear.
		/// </summary>
		/// <param name="pt1">First point</param>
		/// <param name="pt2">Second point</param>
		/// <param name="pt3">Third point</param>
		...
		/// <summary>
		/// Manhattan Distance between two points.  Quicker metric for short distances than the Euclidean one.
		/// </summary>
		/// <param name="pt1">First point</param>
		/// <param name="pt2">Second point</param>
		...
		/// <summary>
		/// Determine the x coordinate where the parabolas with focus at pt1 and pt2 intersect between the two points.
		/// The directrix for both parabolas is the line y = ys.  This is a very specific calculation for the Fortune
		/// algorithm for the Voronoi diagram.
		/// </summary>
		...
		{
			/// Undefined if the focii are identical
		...
		/// <summary>
		/// Compares absolute clockwise angles from the y axis
		/// </summary>
		/// <remarks>
		/// This compares the absolute angle of (ptCenter,pt1) with (ptCenter,pt2) measured clockwise from the
		/// positive y axis.
		/// </remarks>
		...
		{
			PT pt1Rel = new PT(pt1.X - ptCenter.X, pt1.Y - ptCenter.Y);
			PT pt2Rel = new PT(pt2.X - ptCenter.X, pt2.Y - ptCenter.Y);
			int iQuad1 = Geometry.IQuad(pt1Rel);
			int iQuad2 = Geometry.IQuad(pt2Rel);

			/// If they're in different quadrants, only compare the quadrants
			if (iQuad1 != iQuad2)
			{
			...
			return -Geometry.ICcw(ptCenter, pt2, pt1);
		}

		/// <summary>
		/// Determine the center of a circle passing through three points in the plane.  Most
		/// of this is ugly math generated from Mathematica.
		/// </summary>
		/// <param name="pt1">First point</param>
		/// <param name="pt2">Second point</param>
		/// <param name="pt3">Third point</param>
		/// <param name="PtCenter">out parameter returning the circumcenter</param>
		/// <returns>False if the three points lie on a line in which case the circumcenter
		/// is not valid.  True otherwise and the returned point is the circumcenter.</returns>
		public static bool FFindCircumcenter(PT pt1, PT pt2, PT pt3, out PT PtCenter)
		{
			PtCenter = new PT();
			TPT D = (pt1.X - pt3.X) * (pt2.Y - pt3.Y) - (pt2.X - pt3.X) * (pt1.Y - pt3.Y);
